...
#define WIDTH		352 //640
#define	HIGHT		288 //480
#define COUNT		300
...
void init_encoder(int width, int height)
{
	...
	h264_buf = (uint8_t *) malloc( width * height * 2);	
}
...
void encode_frame(uint8_t *yuv_frame, size_t yuv_length) 
{
	...
	{	
		...
		if (write(pipefd[1], h264_buf, h264_length) > 0)
		{
...
int init_mmap(int fd)
{
	...
	for(n_buffer = 0; n_buffer < reqbufs.count; ++n_buffer)
	{
		...
		usr_buf[n_buffer].start = (uint8_t *)mmap(NULL,buf.length,PROT_READ | PROT_WRITE,MAP_SHARED, fd,buf.m.offset);
