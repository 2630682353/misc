...
static void *thread_func(void *arg)
{
	...
	while (1) {
		...
		if (msg_send_syn(WS_TEST1, temp_buf, slen, (void *)&get, &rlen) != 0 ) {
			test.error++;
			continue;
		} else {
			if ((memcmp(temp_buf, get, slen) == 0) && (slen == rlen))
...
//	gettimeofday();
	...
	test.notcompare = 0;
	test.error = 0;
	msg_init(MODULE_DPI, 3, "client1_rcv", "client1_snd");
	signal(SIGINT, sig_hander);
	if (msg_cmd_register(DPI_TEST1, handle_test))
		printf("register error\n");
	if (msg_cmd_register(DPI_TEST2, handle_test_netlink))
		printf("register error\n");

	
	msg_dst_module_register_unix(MODULE_GATEWAY, "server1_rcv");
	msg_dst_module_register_unix(MODULE_WS, "client2_rcv");
	...
	while (1) {
		...
		temp = test.success%6000;
		if (temp < 5)
			TEST_LOG("dpi      success:%d, notcmp:%d, error:%d  time:%d\n", test.success, test.notcompare, test.error, current.tv_sec);
