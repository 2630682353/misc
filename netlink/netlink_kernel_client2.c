...
//      printk(KERN_INFO "thread_function: %d times", ++time_count);  
		...
		temp = success%5000;
		if (temp < 5)
			printk("GATEWAYsuccess:%d, notcmp:%d, error:%d \n", success, notcmp, error);
...
//		__set_current_state(TASK_RUNNING);
		...
		if (msg_send_syn(DPI_KERNEL_TEST1, temp_buf, ilen, &out, &olen) != 0)
			error++;
		else {
			if((memcmp(temp_buf, out, ilen) == 0) && (ilen == olen))
				success++;
...
int32 handle_test3(const int32 cmd, void *ibuf, int32 ilen, void *obuf, int32 *olen)
{
...
int test_netlink_init(void)
{
	...
	msg_cmd_register(GATEWAY_KERNEL_TEST1, handle_test3);
/*
	init_timer(&zc_timer);
...
{
//	del_timer(&zc_timer);
    msg_cmd_unregister(GATEWAY_KERNEL_TEST1);
